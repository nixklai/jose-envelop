...
namespace Envelopes\Tests\ClearEnvelopReaderTest;
...
class ClearEnvelopReaderTest extends TestCase
{
    ...
    public function testVerifyToken()
    {
        $this->envelop = new ClearEnvelop();
        $this->envelop->load($this->no_expiry_jwt);
        $this->envelop->loadKey($this->getPrivateVerifyingKey());
        $this->assertTrue($this->envelop->isValid());
    }

    private function getPrivateVerifyingKey(){
        $key_string = file_get_contents(__DIR__.'/data/private_verifying_key.json');
        return json_decode($key_string, true);
    }
}
